...
#   - 백엔드(Spring, host, profile=dev)  +  프론트(Vite, host, HMR)  +  nginx 프록시(컨테이너)
#   - Ctrl-C 한 번으로 셋 다 종료.  접속: http://localhost:8082
set -euo pipefail
cd "$(dirname "$0")/.."

# 1) .env 로드 (gradlew/npm은 .env 자동 로드 안 함; DB_URL의 '&' 안전 위해 source 대신 line-read)
if [ -f .env ]; then
  set -a
...
# 4) BE/FE를 각자 프로세스그룹으로 백그라운드 기동 (그룹 단위로 깔끔히 종료하기 위함)
echo "▶ 백엔드 bootRun (profile=$SPRING_PROFILES_ACTIVE)"
setsid bash -c 'cd backend && ./gradlew --no-daemon bootRun' & BE_PID=$!
...
cleanup() {
  trap - INT TERM EXIT
  echo; echo "▶ 전체 종료 중…"
  kill -- -"$BE_PID" 2>/dev/null || true   # 프로세스그룹 종료(BE + JVM)
