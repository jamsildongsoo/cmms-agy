...
export default function WorkPermit() {
  ...
  const handleOpenCreate = () => {
    ...
    setEndAt(new Date(Date.now() + 8 * 3600 * 1000).toISOString().substring(0, 16));
  ...
  const handleOpenEdit = async (wp: WorkPermitModel) => {
    ...
    try {
      ...
      setStartAt(w.startAt ? w.startAt.substring(0, 16) : '');
      setEndAt(w.endAt ? w.endAt.substring(0, 16) : '');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 print:border-0 print:bg-transparent print:p-0">
        ...
        <div className="hidden print:block text-center mb-8 border-b-2 border-slate-800 pb-4">
          <h1 className="text-2xl font-extrabold text-slate-900">안 전 작 업 허 가 서 (가로대장)</h1>
              ...
              {permits.length === 0 ? (
                ...
                permits.map((wp) => (
                  <tr key={wp.id} className="border-b border-slate-900 hover:bg-slate-900/30 text-slate-300 print:border-slate-200 print:text-slate-800 print:hover:bg-transparent">
                    ...
                    <td className="p-3 font-mono text-slate-400">{wp.startAt ? wp.startAt.replace('T', ' ').substring(0, 16) : '-'}</td>
                    <td className="p-3 font-mono text-slate-400">{wp.endAt ? wp.endAt.replace('T', ' ').substring(0, 16) : '-'}</td>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto print:absolute print:inset-0 print:bg-white print:p-0">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[95vh] flex flex-col shadow-2xl print:border-0 print:shadow-none print:max-h-none print:w-full print:h-full">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 print:p-0">
              ...
              <div className="space-y-6 print:min-h-screen print:flex print:flex-col print:justify-between">
                
                {/* Print Title Block */}
                <div className="hidden print:block text-center border-b border-slate-300 pb-4 mb-4">
                  <h1 className="text-2xl font-extrabold text-slate-900 tracking-widest">안 전 작 업 허 가 서</h1>
