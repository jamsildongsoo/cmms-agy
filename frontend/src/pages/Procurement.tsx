...
const statusLabel: Record<string, string> = { T: '저장', S: '확정' };
const procLabel: Record<string, string> = { O: '발주', D: '배송중', I: '입고', E: '종료' };
const statusBadgeClass = (s: string) =>
  s === 'S' ? 'bg-emerald-950/40 text-emerald-400 border-emerald-900/60' : 'bg-slate-800 text-slate-300 border-slate-700';
const procBadgeClass = (p?: string | null) => {
  if (!p) return 'bg-slate-900 text-slate-500 border-slate-800';
  if (p === 'E') return 'bg-rose-950/40 text-rose-400 border-rose-900/60';
  if (p === 'I') return 'bg-blue-950/40 text-blue-400 border-blue-900/60';
    ...
    <div className="space-y-6">
            ...
            <tbody>
              ...
              {requests.map(pr => (
                <tr key={pr.id} className="border-t border-slate-800">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${statusBadgeClass(pr.status)}`}>
                      {statusLabel[pr.status] || pr.status} ({pr.status})
                    </span>
                  </td>
                  <td className="p-3">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${procBadgeClass(pr.procStatus)}`}>
                      {pr.procStatus ? `${procLabel[pr.procStatus] || pr.procStatus} (${pr.procStatus})` : '발주대기'}
                    </span>
                  </td>
      ...
      {printPr && (
        <div className="hidden print:block">
          <PrintHeader title="구매요청서" subtitle={printPr.header.id} />
          <table className="w-full text-xs my-4">
            <tbody>
              <tr><td className="p-2 font-bold w-32">PR번호</td><td className="p-2">{printPr.header.id}</td><td className="p-2 font-bold w-32">요청일</td><td className="p-2">{printPr.header.requestDate}</td></tr>
              <tr><td className="p-2 font-bold">요청자</td><td className="p-2">{printPr.header.requesterId}</td><td className="p-2 font-bold">요청유형</td><td className="p-2">{printPr.header.requestType || '-'}</td></tr>
              <tr><td className="p-2 font-bold">플랜트</td><td className="p-2">{printPr.header.plantId}</td><td className="p-2 font-bold">입고 저장소</td><td className="p-2">{printPr.header.warehouseId}</td></tr>
            </tbody>
          </table>
          <table className="w-full text-xs border-t border-b border-slate-700">
            <thead><tr className="bg-slate-100"><th className="p-2 text-left">No</th><th className="p-2 text-left">자재</th><th className="p-2 text-right">수량</th><th className="p-2 text-left">단위</th><th className="p-2 text-left">비고</th></tr></thead>
            <tbody>
              {printPr.items.map((it, i) => (
                <tr key={i} className="border-t border-slate-300">
                  <td className="p-2">{i + 1}</td>
                  <td className="p-2">{it.inventoryId}</td>
