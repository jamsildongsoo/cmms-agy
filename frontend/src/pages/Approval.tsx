...
interface ApprovalStepModel {
  ...
  approvalResult: string; // T: 대기, P: 진행, A: 승인, R: 반려
...
export default function Approval() {
  ...
  const getStepTypeLabel = (t: string) => {
    return { D: '기안', A: '결재', G: '합의', R: '참조' }[t] || t;
  };
  ...
  const isMyTurn = selectedApproval?.status === 'P' && approvalSteps.some(
    step => step.approverId === user?.id && step.approvalResult === 'P'
  );
        ...
        <div className="p-1 border-t border-slate-850 text-[8px] print:border-slate-300">
          ...
          {step.actionAt && (
            <div className="text-slate-500 font-mono text-[7px] mt-0.5 print:text-slate-600">
              {step.actionAt.replace('T', ' ').substring(0, 16)}
            </div>
          )}
        </div>
      </div>
              ...
              {approvals.length === 0 ? (
                ...
                approvals.map((app) => (
                  <tr key={app.id} className="border-b border-slate-900 hover:bg-slate-900/30 text-slate-300">
                    ...
                    <td className="p-3 font-mono text-slate-400">{app.createdAt.replace('T', ' ').substring(0, 16)}</td>
                    <td className="p-3">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl print:border-0 print:shadow-none print:max-h-none print:w-full print:h-full">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 print:p-0 print:bg-white print:text-black">
              ...
              <div className="hidden print:block text-center border-b border-slate-350 pb-4 mb-6">
                <h1 className="text-2xl font-extrabold tracking-widest text-slate-900">결 재 품 의 서</h1>
                      ...
                      {approvalSteps.map((step) => {
                        ...
                        return (
                          <tr key={step.stepNo} className="border-b border-slate-900 text-slate-300 print:border-slate-200 print:text-black">
                            ...
                            <td className="p-2">
                              <span className={step.approvalResult === 'A' ? 'text-emerald-400 font-semibold' : step.approvalResult === 'R' ? 'text-rose-400 font-semibold' : 'text-slate-500'}>
                                {step.approvalResult === 'A' ? '승인' : step.approvalResult === 'R' ? '반려' : step.approvalResult === 'P' ? '진행중' : '대기'}
                              </span>
                            </td>
                            <td className="p-2 text-slate-400 font-sans italic">{step.comments || '-'}</td>
                            <td className="p-2 font-mono text-slate-500">{step.actionAt ? step.actionAt.replace('T', ' ').substring(0, 16) : '-'}</td>
                          </tr>
                        );
                      })}
                    </tbody>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl max-h-[85vh] flex flex-col shadow-2xl">
            ...
            <div className="p-6 border-t border-slate-800 flex justify-end gap-2 shrink-0">
              ...
              <button onClick={handleSaveDraft} disabled={isLoading} className="bg-blue-600 hover:bg-blue-500 text-white rounded-lg py-2 px-5 border-0 cursor-pointer disabled:opacity-50">기안 상신</button>
            </div>
          </div>
        </div>
      )}
